...
| 02 Set 2026 | PO pediu auditoria geral e reportou vídeos incorporados (YouTube, via IFrame API em `VSLVideo.tsx` na homepage e `SalaVideoPlayer.tsx` em `/salas`) sem autoplay, apesar de ambos os componentes terem `playerVars: { autoplay: 1, mute: 1, playsinline: 1 }` correctos. Causa raiz: o header `Permissions-Policy` em `next.config.js` (adicionado em VOL03-10D, "Hardening: Security headers") listava apenas `camera=(), microphone=(), geolocation=()` — ao omitir a directiva `autoplay`, o browser aplica o valor por omissão `self`, que **não delega autoplay a iframes de origem diferente** (o embed do YouTube), independentemente do atributo `allow="autoplay"` que a própria API do YouTube já adiciona ao iframe: a política do documento de topo tem sempre precedência sobre o que o iframe pede. Corrigido acrescentando `autoplay=(self "https://www.youtube.com" "https://www.youtube-nocookie.com")` — mesmas origens já permitidas em `frame-src` no CSP. Aproveitado para corrigir um bug secundário em `SalaVideoPlayer.tsx`: faltava o guard `!document.getElementById("youtube-iframe-api")` antes de injectar o `<script>` da IFrame API (já existente em `VSLVideo.tsx`), o que injectava múltiplas cópias do script e sobrescrevia `window.onYouTubeIframeAPIReady` em corrida caso o componente remontasse via navegação client-side. Auditoria mais ampla: confirmado que estes são os únicos 2 pontos de embed cross-origin nas páginas públicas (`/`, `/salas`, `/obrigado`) — sem outros vídeos ou iframes afectados | `next.config.js`, `src/components/SalaVideoPlayer.tsx` | Não |
| 02 Set 2026 | PO confirmou que a correcção do `Permissions-Policy` acima (já em produção, confirmado por leitura directa dos response headers de `https://www.azulcowork.com/`) não resolveu — autoplay continuava a falhar. Verificação ao vivo com browser automatizado revelou 2 causas adicionais, ambas de robustez do lado do cliente (não de configuração de servidor): (1) numa das cargas de página, o pedido a `https://www.youtube.com/iframe_api` foi respondido pela Google com um redirect para uma página de verificação anti-bot (`google.com/sorry/...`), que o CSP bloqueia por não incluir `google.com` em `script-src` — sem qualquer nova tentativa, `window.YT` nunca fica definido e o vídeo nunca aparece nessa carga. Corrigido com retry automático (até 2 tentativas, 6s de intervalo) em ambos os componentes: se `window.YT.Player` não ficar pronto a tempo, remove o `<script>` preso e tenta de novo. (2) `playerVars: { autoplay: 1 }` sozinho é sabidamente inconsistente entre browsers quando o player é criado via JS API (em vez de um embed estático já presente no HTML no primeiro parse) — corrigido chamando `mute()` + `playVideo()` explicitamente dentro do handler `onReady` de ambos os players, como reforço ao parâmetro `autoplay`. Confirmado via inspecção do iframe gerado em produção: `src` inclui `autoplay=1&mute=1&playsinline=1` e `allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"` — configuração correcta em todos os pontos verificáveis | `src/components/VSLVideo.tsx`, `src/components/SalaVideoPlayer.tsx` | Não |
| 03 Set 2026 | PO continuou a reportar "os vídeos não funcionam" mesmo após a correcção de robustez acima. Verificação ao vivo confirmou que o retry funciona (após ~8-13s a API do YouTube fica pronta e o iframe monta correctamente), mas até lá o `<div>` do player ficava um rectângulo vazio sobre fundo escuro — indistinguível de "quebrado" para quem testa nos primeiros segundos, especialmente em redes mais lentas (público-alvo em Luanda). Corrigido acrescentando uma capa (thumbnail real do vídeo via `https://i.ytimg.com/vi/<id>/maxresdefault.jpg`, com `animate-pulse`) visível em ambos os componentes até o evento `onReady` do player disparar (`playerReady` state) — nunca mais aparenta estar vazio/quebrado enquanto a API carrega ou tenta novamente em segundo plano. Acrescentado `https://i.ytimg.com` a `img-src` no CSP (`next.config.js`), sem o qual a própria imagem de capa seria bloqueada | `src/components/VSLVideo.tsx`, `src/components/SalaVideoPlayer.tsx`, `next.config.js` | Não |
| 15 Set 2026 | PO reportou "erro ao converter lead em cliente" (screenshot: modal "Converter em Cliente" em Leads Salas, categoria "Registo normal — Salas de Reunião", mensagem genérica "Erro ao converter."). Causa raiz: `src/app/api/room-booking-leads/[id]/convert/route.ts` passava `leadId: lead.id` para `addTimeline()`, mas `lead` ali é um `RoomBookingLead`, enquanto `Timeline.leadId` tem uma FK real para a tabela `Lead` (CRM) — ver `prisma/migrations/20240114000000_employees_notifications_fks/migration.sql`, que criou `Timeline_leadId_fkey FOREIGN KEY ("leadId") REFERENCES "Lead"("id")`. Qualquer conversão de lead de sala violava esta constraint dentro da `$transaction`, revertendo tudo (a Company nem chegava a ser criada) — e como a rota não tinha `try/catch`, a excepção propagava sem tratamento, o Next.js devolvia um 500 sem JSON estruturado, o frontend caía no fallback genérico "Erro ao converter.", e sem `Sentry.captureException` o erro nunca aparecia no Sentry. Corrigido removendo o `leadId` incorrecto (o ID do RoomBookingLead já fica registado no campo `description` do timeline, não se perde informação) e acrescentando `try/catch` + `Sentry.captureException` + mensagem de erro real na resposta JSON. **Mesmo bug encontrado por auditoria em `src/app/api/room-booking-leads/[id]/to-reservation/route.ts`** (idêntico: `lead` também é `RoomBookingLead`) — corrigido da mesma forma, preventivamente, antes de ser reportado | `src/app/api/room-booking-leads/[id]/convert/route.ts`, `src/app/api/room-booking-leads/[id]/to-reservation/route.ts` | Não |
| 22 Set 2026 | PO pediu auditoria geral de segurança e erros ao site completo. Cobertura: RBAC em todas as 198 API Routes (via `requireRole`/`requireSession` no admin e `requirePortalSession`/`requirePortalRoleAndCompany` no portal — confirmado isolamento multi-tenant correcto em todas as rotas `portal/*`, incl. 404 genérico em vez de 403 para não revelar recursos de outra empresa), segredos (nenhum hardcoded; `.env*` correctamente fora do git; `JWT_SECRET`/`CRON_SECRET` falham fechados quando ausentes), XSS (`dangerouslySetInnerHTML` não é usado em lado nenhum do código), SQL (as 2 únicas queries raw — `auth/login` e `admin/init-pilot` — são parametrizadas via template tag do Prisma, sem concatenação de string), upload (`api/upload` valida MIME + limite de 10 MB), CSP/security headers, e reincidência da classe de bug corrigida em 15 Set (`$transaction` sem `try/catch` → 500 sem JSON estruturado e sem Sentry). Encontrada a mesma classe de bug (transacção sem `try/catch`) em mais 4 rotas financeiras/administrativas que nunca tinham sido reportadas como avariadas — corrigidas preventivamente com o mesmo padrão (`try/catch` + `Sentry.captureException` + mensagem de erro real): criação de pagamento, actualização de pagamento, criação de factura, e preçário de sala (upsert em lote). **Achado crítico não corrigido nesta sessão (requer decisão do PO):** `src/app/api/admin/init-pilot/route.ts` continua deployado em produção — endpoint com o próprio comentário "ENDPOINT TEMPORÁRIO — APAGAR APÓS PRIMEIRO USO" que (re)cria/reactiva as contas `ernesto@azulcowork.com` e `operacoes@azulcowork.com` com password nova devolvida em JSON, protegido apenas por comparação `!==` (não constant-time) contra `PILOT_SETUP_SECRET`; enquanto este ficheiro existir e essa env var estiver definida na Vercel, é uma porta permanente para reset de password admin sem 2FA. Recomendação: apagar o ficheiro (ou pelo menos remover/rodar `PILOT_SETUP_SECRET` na Vercel) assim que confirmado que já não é necessário. **Achado médio (arquitectural, não corrigido — fora do âmbito de uma correcção pontual):** o rate limiting (`src/lib/rateLimit.ts`) usa `Map` em memória por instância — já auto-documentado no ficheiro como "substituir por Upstash Redis em produção com múltiplas instâncias"; em Vercel (serverless, múltiplas instâncias/regiões) os limites por IP são efectivamente por-instância, não globais, pelo que a protecção real contra brute-force/spam é mais fraca do que os números sugerem (mitigado por bcrypt + mensagens genéricas + auditoria, mas não eliminado) — candidato a backlog pós-piloto. Nenhuma outra rota (das 198) ficou sem protecção de autenticação adequada; as únicas rotas sem `requireRole`/`requirePortalSession` são as legitimamente públicas (login, magic link, cron com `CRON_SECRET`, e 3 stubs legacy em `/api/rooms*` que só devolvem 404/410) | `src/app/api/payments/route.ts`, `src/app/api/payments/[id]/route.ts`, `src/app/api/invoices/route.ts`, `src/app/api/admin/room-pricing/route.ts` | Não |
